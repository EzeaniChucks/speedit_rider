...
const CreateAccountScreen = ({ navigation }) => {
    ...
    const handleContinue = () => {
        if (!firstName.trim() || !lastName.trim() || !email.trim() || !phone.trim() || !residentialAddress.trim()) {
            Alert.alert("Validation Error", "All personal and address fields are required.");
            return;
        }
        ...
        if (!plateNumber.trim() || !vehicleColor.trim() || !vehicleModel.trim()) {
            Alert.alert("Validation Error", "All vehicle detail fields are required.");
            return;
        }
        if (!/\S+@\S+\.\S+/.test(email)) {
        ...
        if (!/^\d{10,11}$/.test(phone)) {
             Alert.alert("Validation Error", "Please enter a valid phone number (10 or 11 digits after +234).");
             return;
        }
    ...
    const paperInputTheme = { roundness: 5, colors: { text: '#000', placeholder: '#666' } }; // Added text and placeholder color for consistency
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.container}>
                ...
                <TouchableOpacity style={styles.dropdown} onPress={() => setModalVisible(true)}>
                    <Text style={styles.dropdownText}>{selectedVehicle || 'Select Vehicle Type'}</Text>
                </TouchableOpacity>
               <Modal
                    transparent={true}
                    animationType="slide"
                ...
                <TouchableOpacity onPress={handleLogin} style={styles.signInLinkContainer}>
...
const styles = StyleSheet.create({
    ...
    phoneCode: { // For the "+234" TextInput
    ...
    modalContent: { // Wrap actual modal content for better styling
