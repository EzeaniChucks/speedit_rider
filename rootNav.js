...
const RootNavigation = () => {
  ...
  async function requestUserPermissionAndGetToken(dispatch) { // Pass dispatch if storing in Redux
  let fcmToken = null;
  ...
  if (Platform.OS === 'ios') {
    ...
    if (enabled) {
      ...
      console.log('User has not granted permission for push notifications (iOS).');
      Alert.alert("Permission Denied", "You will not receive notifications unless you enable them in settings.");
    }
    ...
    } else { // Android 12 and below (permission usually granted by default)
...
async function getFcmTokenInternal(dispatch) { // Pass dispatch if storing in Redux
  try {
    ...
    if (token) {
      ...
      console.log('Failed to get FCM token. messaging().getToken() returned null/undefined.');
      Alert.alert("FCM Error", "Could not retrieve a messaging token. Notifications might not work.");
      return null;
    }
  } catch (error) {
