...
console.log("Transactions:", transactions, "Status:", transactionStatus, "Error:", transactionError);
  ...
  const renderActivityItem = ({ item }) => (
    ...
    >
      <AntDesignIcon name={item.type === 'credit' ? "pluscircleo" : "minuscircleo"} size={24} color={item.type === 'credit' ? 'teal' : 'crimson'} />
      <View style={styles.activityDetails}>
        <Text style={styles.activityTitle} numberOfLines={1}>{item.description || `Transaction ${item.id}`}</Text>
        <Text style={styles.activityAmount}>
          {item.type === 'credit' ? '+' : '-'} ${parseFloat(item.amount).toFixed(2)}
        </Text>
      </View>
      <Text style={styles.activityDate}>{new Date(item.createdAt || item.date).toLocaleDateString()}</Text>
    </TouchableOpacity>
  );
    ...
    <View style={styles.container}>
        <HStack justifyContent={'space-between'} alignItems={'center'} mb={2}>
            ...
            <Pressable 
                ...
                rounded="md"
            >
                <Text style={{fontSize:14, color:'teal', fontWeight: 'bold'}}>Show More</Text>
            </Pressable>
        </HStack>
        {transactionStatus === 'loading' && transactions.length === 0 && <ActivityIndicator color="teal"/>}
        {transactionError && transactions.length === 0 && <Text>Error loading activities.</Text>}
        ...
        {transactionStatus !== 'loading' && transactions.length === 0 && <Text>No recent activity.</Text>}
    </View>
  );
};
