...
const DeliveryScreen = ({navigation}) => {
    // const navigation = useNavigation();
      ...
      const handleMarkDelivered = async () => {
          try {
              ...
              Alert.alert('Order Complete!', 'You have successfully delivered the order.', [
                  { text: 'OK', onPress: () => navigation.navigate('RiderActive') } // Go back to the main screen
              ]);
          } catch (err) {
  ...
  return (
   ...
   <HStack justifyContent={'space-between'} alignItems="center" mb={4}>
              ...
              <Text style={styles.header} numberOfLines={1}>Deliver: {order.id}</Text>
          </HStack>
        <Text style={styles.deliveryTitle}>Deliver to:</Text>
              
              <View style={styles.card}>
                <Text style={styles.deliverName}>{order.customer.firstName} {order.customer.lastName}</Text>
         ...
         <Text style={styles.itemsTitle}>Items ({order.items.length})</Text>
               ...
               <Text style={styles.greenText}>Given {order.items.length} item(s) to customer</Text>
               <Text style={styles.infoText}>
                 {order.specialInstructions ? `Special Instructions: ${order.specialInstructions}` : "No special instructions for delivery."}
               </Text>
             </ScrollView>
       
             <TouchableOpacity style={styles.button} onPress={handleMarkDelivered} disabled={isUpdatingStatus}>
               {isUpdatingStatus ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Mark Order Delivered</Text>}
             </TouchableOpacity>
           </View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#f9f9f9', padding: 16, borderRadius: 8, marginVertical: 8, },
  ...
  itemContainer: { flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderColor: '#ddd', paddingVertical: 12 },
  itemName: { fontSize: 16, },
  greenText: { color: 'green', marginVertical: 8, fontWeight: 'bold' },
  infoText: { fontSize: 12, color: 'gray', marginVertical: 8, fontStyle: 'italic' },
  button: { backgroundColor: '#FF3D00', padding: 16, borderRadius: 8, alignItems: 'center', margin: 16 },
  container: {
    flex: 1,
    backgroundColor: '#fff',
  },
